...
ここでmain内の説明に戻る。
12行目でレジスタ$spを復活させる
13行目でレジスタ$v0の保持するアドレスをレジスタ$a0に格納する。
14行目でsyscallでint型を表示する為にレジスタ$v0に即値1を格納する。
15行目でレジスタ$a0の保持する値が表示される。
16行目でstrが指すアドレスをレジスタ$a0に格納する。
17行目でsyscallでstring型を表示する為にレジスタ$v0に即値４を格納する。
18行目で改行処理が行われる。
19行目でレジスタ$t0から１引いたものをレジスタ$t0に格納する。
20行目でレジスタ$t0の保持する値が０より大きいか等しいならばラベルloopへ戻る
21行目で呼び出し側に戻り終了である。
```
